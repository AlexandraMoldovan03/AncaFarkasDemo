...
;(async () => {
  try {
    ...
    if (uErr) console.error('getUser err', uErr)
    if (!user) { location.href = '/auth.html'; return }
    document.getElementById('userEmail').textContent = user.email

    const list = document.getElementById('courses')
    // curăță skeleton-urile
    list.innerHTML = ''

    // 1) produse (pentru nume)
    ...
    list.innerHTML = data.map(p => {
      ...
      const productName = nameById[String(p.item_id)] ?? `Produs #${p.item_id}`
      return `
        <li>
          <div>
            <strong>${productName}</strong> — ${p.status} — ${price} — <small>${when}</small>
          </div>
          <div style="margin-top:6px;">
            ${canDownload ? `
              <button class="dl btn btn-sm btn-primary" data-item="${p.item_id}">Descarcă</button>
              <button class="email btn btn-sm btn-outline-secondary" data-item="${p.item_id}">Trimite pe email</button>
            ` : `<span class="text-muted">Plata nu e finalizată.</span>`}
          </div>
        </li>
      `
    }).join('')
    ...
    list.addEventListener('click', async (ev) => {
      ...
      if (btn.classList.contains('dl')) {
        try {
          ...
          btn.disabled = false; btn.textContent = 'Descarcă'
        }
      }
    ...
    const list = document.getElementById('courses')
    if (list) list.innerHTML = `<li>Eroare: ${e.message ?? e}</li>`
  }
})()
